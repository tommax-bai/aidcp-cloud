 ...
 * 所以规范落点是这里一份，`content-authority-http.ts` 在接线那一轮改指过来
 * （见 {@link file://./content-media-usage-http.ts} 的欠账登记；本轮不改它，它是并行热点的只读件）。
 *
...
} from '../kernel/content-port-error.js';
import {
  InternalHttpError,
...
function clientFailureAsContentPortError(
  ...
  switch (code) {
    ...
    case 'api_direct_version_unsupported':
      ...
      // 契约版本、照样会失败。判成回落只会把一次通道级配置错误伪装成一次能力缺口。
      return new ContentPortError('remote_error', operation, `${code}: ${message}`);
    default:
      return new ContentPortError(
        'remote_error',
