-- aidcp:kind=expand
-- aidcp:objects=table:automation_sync_read_consumer_checkpoint,column:automation_sync_read_consumer_checkpoint.execution_target
-- aidcp:objects=column:automation_sync_read_consumer_checkpoint.consumer,column:automation_sync_read_consumer_checkpoint.stream
-- aidcp:objects=column:automation_sync_read_consumer_checkpoint.applied_cursor,column:automation_sync_read_consumer_checkpoint.payload_digest
-- aidcp:objects=column:automation_sync_read_consumer_checkpoint.source_as_of_ms,column:automation_sync_read_consumer_checkpoint.last_observed_at_ms
-- aidcp:objects=column:automation_sync_read_consumer_checkpoint.fresh_until_ms,column:automation_sync_read_consumer_checkpoint.last_applied_at_ms
-- aidcp:objects=column:automation_sync_read_consumer_checkpoint.state,column:automation_sync_read_consumer_checkpoint.last_error
-- aidcp:objects=column:automation_sync_read_consumer_checkpoint.updated_at
--
-- change split-cloud-api-composition-root-4b：automation 消费 api 快照的本地恢复状态。
...
CREATE TABLE IF NOT EXISTS automation_sync_read_consumer_checkpoint (
  execution_target   TEXT NOT NULL CHECK (execution_target IN ('dev', 'ol')),
  consumer           TEXT NOT NULL CHECK (consumer = 'automation'),
