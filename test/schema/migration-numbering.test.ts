...
import { numericPrefix, parseMigrationHeader, versionOf } from '../../src/schema/migration-plan.js';
...
test('除已登记的四组外不存在新的同号；四组的文件名集合与相对顺序与 README 一致', async () => {
  ...
  for (const [prefix, expected] of Object.entries(FROZEN_COLLISIONS)) {
    // loadMigrationFiles 已按复合序排好，故这里的顺序就是执行器的实际顺序。
...
test('每个迁移文件都带 kind 与 objects 两段头声明', async () => {
  ...
  for (const f of files) {
    ...
    const raw = await readFile(`${migrationsDir()}/${f.name}`, 'utf8');
