...
test('三档每日配额符合 risk-control 第 6 节', () => {
  assert.deepEqual(DAILY_QUOTAS.conservative, { view: 80, like: 20, collect: 10, comment: 3, follow: 5, publish: 1, comment_like: 3 });
  assert.deepEqual(DAILY_QUOTAS.normal, { view: 150, like: 50, collect: 25, comment: 8, follow: 15, publish: 1, comment_like: 6 });
  assert.deepEqual(DAILY_QUOTAS.aggressive, { view: 300, like: 100, collect: 50, comment: 15, follow: 30, publish: 2, comment_like: 12 });
});
